...
const checkIcon = <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
const xIcon     = <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}><path strokeLinecap="round" strokeLinejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
const calIcon   = <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
const creditIcon= <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
const starIcon  = <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}><path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>
const aiIcon    = <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
const bellIcon  = <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>

const TYPE_META = {
  BOOKING_REQUEST:          { icon: calIcon,    color: 'bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400',           dot: 'bg-blue-500'    },
  BOOKING_ACCEPTED:         { icon: checkIcon,  color: 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400', dot: 'bg-emerald-500' },
  BOOKING_DECLINED:         { icon: xIcon,      color: 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400',               dot: 'bg-red-500'     },
  BOOKING_CANCELLED:        { icon: xIcon,      color: 'bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400',    dot: 'bg-orange-500'  },
  BOOKING_COMPLETED:        { icon: checkIcon,  color: 'bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-400',           dot: 'bg-teal-500'    },
  CREDITS_RELEASED:         { icon: creditIcon, color: 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400', dot: 'bg-emerald-500' },
  CREDITS_REFUNDED:         { icon: creditIcon, color: 'bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400',           dot: 'bg-blue-500'    },
  CREDITS_WELCOME_BONUS:    { icon: creditIcon, color: 'bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400',   dot: 'bg-purple-500'  },
  CREDITS_ADMIN_ADJUSTMENT: { icon: creditIcon, color: 'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-600 dark:text-yellow-400',   dot: 'bg-yellow-500'  },
  NEW_REVIEW:               { icon: starIcon,   color: 'bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400',       dot: 'bg-amber-500'   },
  AI_MATCH:                 { icon: aiIcon,     color: 'bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-400',   dot: 'bg-violet-500'  },
  DEFAULT:                  { icon: bellIcon,   color: 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400',             dot: 'bg-gray-400'    },
}
      ...
      {isOpen &&
        createPortal(
          <div
            ...
            style={{ top: dropdownCoords.top, left: dropdownCoords.left, position: 'fixed', animation: 'notif-slide-in 0.18s ease' }}
          >
            <style>{`
...
function NotificationItem({ notification: n, isDeleting, onClick, onDelete }) {
  ...
  return (
    ...
    >
      {/* Unread dot */}
      {!n.isRead && (
        <span className={`absolute left-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full ${meta.dot} shrink-0`} />
      )}

      {/* Type icon */}
      <div className={`mt-0.5 w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${meta.color}`}>
        {meta.icon}
      </div>

      {/* Content */}
      <div className="flex-1 min-w-0">
        <p className={`text-[13px] font-semibold leading-snug mb-0.5 ${n.isRead ? 'text-gray-700 dark:text-gray-300' : 'text-gray-900 dark:text-white'}`}>
          {n.title}
        </p>
  ...
  return (
    <div className="px-4 py-10 flex flex-col items-center gap-3 text-gray-400 dark:text-gray-500">
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">No notifications right now.</p>
      </div>
    </div>
  )
}

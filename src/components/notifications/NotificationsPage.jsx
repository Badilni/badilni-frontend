...
// Shared SVG icons
const I = {
  check:  <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  x:      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  cal:    <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
  credit: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
  star:   <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" /></svg>,
  ai:     <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>,
  bell:   <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>,
}

const TYPE_META = {
  BOOKING_REQUEST:          { label: 'Booking Request',    icon: I.cal,    iconBg: 'bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400',            dot: 'bg-blue-500',    badge: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'           },
  BOOKING_ACCEPTED:         { label: 'Booking Accepted',   icon: I.check,  iconBg: 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400',  dot: 'bg-emerald-500', badge: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' },
  BOOKING_DECLINED:         { label: 'Booking Declined',   icon: I.x,      iconBg: 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400',                  dot: 'bg-red-500',     badge: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300'               },
  BOOKING_CANCELLED:        { label: 'Booking Cancelled',  icon: I.x,      iconBg: 'bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400',      dot: 'bg-orange-500',  badge: 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300'   },
  BOOKING_COMPLETED:        { label: 'Session Completed',  icon: I.check,  iconBg: 'bg-teal-100 dark:bg-teal-900/40 text-teal-600 dark:text-teal-400',              dot: 'bg-teal-500',    badge: 'bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300'           },
  CREDITS_RELEASED:         { label: 'Credits Received',   icon: I.credit, iconBg: 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400',  dot: 'bg-emerald-500', badge: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' },
  CREDITS_REFUNDED:         { label: 'Credits Refunded',   icon: I.credit, iconBg: 'bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400',             dot: 'bg-blue-500',    badge: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'           },
  CREDITS_WELCOME_BONUS:    { label: 'Welcome Bonus',      icon: I.credit, iconBg: 'bg-purple-100 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400',      dot: 'bg-purple-500',  badge: 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300'   },
  CREDITS_ADMIN_ADJUSTMENT: { label: 'Credits Adjustment', icon: I.credit, iconBg: 'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-600 dark:text-yellow-400',      dot: 'bg-yellow-500',  badge: 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300'   },
  NEW_REVIEW:               { label: 'New Review',         icon: I.star,   iconBg: 'bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400',          dot: 'bg-amber-500',   badge: 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300'       },
  AI_MATCH:                 { label: 'AI Match',           icon: I.ai,     iconBg: 'bg-violet-100 dark:bg-violet-900/40 text-violet-600 dark:text-violet-400',      dot: 'bg-violet-500',  badge: 'bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300'   },
  DEFAULT:                  { label: 'Notification',       icon: I.bell,   iconBg: 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400',               dot: 'bg-gray-400',    badge: 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300'             },
}
...
export default function NotificationsPage() {
  ...
  const selected = notifications.find((n) => n._id === selectedId) ?? filtered[0] ?? null
              ...
              {FILTER_TABS.map((tab) => {
                ...
                return (
                  ...
                  >
                    {tab.label}
                    {count > 0 && (
                      <span className={`ml-1 ${activeFilter === tab.key ? 'opacity-80' : 'opacity-60'}`}>
                        {count}
                      </span>
                    )}
                  </button>
...
function SidebarItem({ notification: n, isActive, isDeleting, onClick, onDelete }) {
  ...
  return (
    ...
    >
      ...
      {/* Icon */}
      <div className={`mt-0.5 w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${meta.iconBg}`}>
        {meta.icon}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-baseline justify-between gap-1 mb-0.5">
          <p className={`text-[13px] font-semibold truncate ${n.isRead ? 'text-gray-700 dark:text-gray-300' : 'text-gray-900 dark:text-white'}`}>
            {n.title}
          </p>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 md:p-10 flex flex-col items-center justify-start">
        <div className="w-full max-w-lg">
          {/* Icon */}
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 mx-auto ${meta.iconBg}`}>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 p-5 space-y-3 shadow-sm">
            ...
            <MetaRow label="Status" value={
              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${n.isRead ? 'bg-gray-100 dark:bg-slate-800 text-gray-500' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'}`}>
                {n.isRead ? 'Read' : 'Unread'}
              </span>
            } />
            <MetaRow label="Received" value={new Date(n.createdAt).toLocaleString()} />
            {n.relatedType && <MetaRow label="Related to" value={n.relatedType} />}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 text-gray-400 dark:text-gray-600">
      ...
      <div className="text-center">
        <p className="font-semibold text-gray-700 dark:text-gray-300">Select a notification</p>
        <p className="text-sm mt-1">Pick one from the list to view its details</p>
      </div>
    </div>
  )
}
...
function ErrorState({ onRetry }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-16 px-6 text-center">
      ...
      <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">Failed to load</p>
      <button
        onClick={onRetry}

...
function CategoryCard({ category, isActive, onClick }) {
  return (
    ...
    >
      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-3 transition-transform duration-200 group-hover:scale-110 ${category.bg} ${category.iconColor}`}>
        {category.icon}
      </div>
      <span className="text-sm font-bold text-gray-800 dark:text-gray-100 mb-1">{category.label}</span>
      <span className="text-xs text-gray-400 dark:text-gray-500">{category.count} Mentors</span>
    </button>
  )
}
  ...
  return (
    <section className="mb-16">
      <div className="flex justify-between items-end mb-8">
        <div>
          <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-1">Explore Categories</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400">Find the right path for your next career move</p>
        </div>

  ...
  return (
    ...
    >
      ...
      <div className="relative h-52 overflow-hidden bg-gray-100 dark:bg-slate-800">
        ...
        {/* Fallback avatar */}
        <div
          className={`absolute inset-0 hidden items-center justify-center bg-gradient-to-br ${mentor.accentColor} text-white text-5xl font-bold`}
        >
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {mentor.tags.map((tag) => (
            <span
              key={tag}
              className={`text-xs font-semibold px-2.5 py-1 rounded-lg ${mentor.tagColor}`}
            >
              {tag}
            </span>
          ))}
        ...
        <div className="pt-4 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
          ...
          <button
            className={`px-5 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r ${mentor.accentColor} hover:shadow-lg hover:brightness-110 transition-all duration-200 active:scale-95`}
          >
            View Profile
          </button>
        </div>

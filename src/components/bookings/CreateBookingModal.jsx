 ...
 */
export default function CreateBookingModal({ open, onClose, listingId, requestId, onSuccess }) {
  ...
  const handleSubmit = (e) => {
    ...
    mutate(
      ...
      {
        onSuccess: (data) => {
          setForm({ scheduledAt: '', durationHours: '', note: '', images: { existing: [], files: [] } })
          onSuccess?.(data)
          onClose()
        },
      }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-black text-gray-900 dark:text-white">Book a Session</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-gray-900 dark:text-white"
              />
              {fieldErrors.durationHours && <p className="text-xs text-red-500 mt-1">{fieldErrors.durationHours}</p>}
            </div>
          </div>

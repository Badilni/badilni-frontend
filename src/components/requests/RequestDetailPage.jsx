...
export default function RequestDetailPage() {
  ...
  if (error) return <div className="p-10 text-center text-red-500">Error loading request.</div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 lg:p-10">
      <div className="bg-[var(--whiteBackground)] dark:bg-slate-900 rounded-3xl p-8 border border-gray-100 dark:border-slate-800 shadow-sm">
        ...
        {/* Title & Metadata */}
        <h1 className="text-3xl font-black text-gray-900 dark:text-white mb-4">{request.title}</h1>
        ...
        {request.referenceImages?.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {request.referenceImages.map((img, idx) => (
              <img key={idx} src={img.url} className="rounded-2xl w-full h-64 object-cover" />
            ))}
          </div>
        )}
        ...
        <div className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-slate-800 rounded-2xl">
          <img src={request.user?.avatar?.url} className="w-12 h-12 rounded-full" />
          <div>

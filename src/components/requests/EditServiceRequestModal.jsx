...
import { useEditServiceRequest } from '../../hooks/Timeline/useServiceRequestMutations'
import { toDatetimeLocalValue } from '../../utils/toDatetimeLocalValue'

export default function EditServiceRequestModal({ open, request, onClose }) {
  const { mutate, isPending, error } = useEditServiceRequest()
  ...
  const initialValues = {
    ...
    images: { existing: request.referenceImages ?? [], files: [] },
  }
    ...
    mutate(
      {
        ...
        payload: {
          ...
          keepImageIds: form.images?.existing?.map((img) => img._id),
        },
      },
      { onSuccess: () => onClose() }
    )
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-black text-gray-900 dark:text-white">Edit Request</h2>
        ...
        {error && !Object.keys(fieldErrors).length && (
          <div className="mb-4 text-sm font-semibold text-red-600 bg-red-50 dark:bg-red-950/30 rounded-xl px-4 py-3">
            {error.message || "Couldn't update your request. Try again."}
          </div>
        )}

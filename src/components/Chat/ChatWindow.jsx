...
const ChatWindow = ({
  ...
  return (
    <div
      className={`flex-1 h-full bg-[var(--background-light)] flex flex-col p-4 md:p-6 transition-all duration-300 ${
        viewMode === 'chat'
          ? 'flex'
          : viewMode === 'sidebar'
            ? 'hidden md:flex'
    ...
    >
      ...
      <div className="flex justify-between items-center pb-4 border-b border-[var(--gray-text)]/10 dark:border-[var(--border-color)] shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="text-base md:text-lg font-bold text-[var(--black-text)] truncate">
              {currentChat.name}
            </h3>
      ...
      <div className="flex-1 overflow-y-auto my-4 space-y-4 pr-1 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {currentChat.messages.map((msg) => (
          <div
            key={msg.id}
            className={`flex flex-col ${msg.isMe ? 'items-end w-full' : 'items-start max-w-[85%] md:max-w-[70%]'}`}
          >
            <span className="text-[10px] text-[var(--gray-text)] mb-0.5 px-2">
              {msg.time}
            </span>
            <div className="flex items-end gap-2">
              {!msg.isMe && (
                <img
                  src={msg.avatar || currentChat.img}
                  className="w-7 h-7 rounded-full object-cover mb-1 shrink-0"
                  alt=""
                />
              )}
              <div
                className={`p-3 text-xs md:text-sm rounded-2xl ${
                  msg.isMe
      ...
      >
        ...
        <button
          type="submit"
          className="p-2.5 bg-[var(--primary-light)] text-white rounded-xl hover:opacity-90 transition-opacity"
        >
          <FiSend size={14} />
        </button>
      </form>

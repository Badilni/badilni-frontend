...
const HeaderChatDropdown = () => {
    ...
    if (isDropdownOpen) {
      ...
      getConversations(1, 20)
        .then((data) => {
          ...
          const chatList = Array.isArray(data) ? data : (data.conversations || data.data || []);
          setChats(chatList);
        })
        .catch((err) => {
  ...
  const handleSelectChat = async (chat) => {
    ...
    try {
      ...
      setSelectedChat(prev => ({ ...prev, messages: Array.isArray(messagesData) ? messagesData : [] }));
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const newMessage = await sendMessage(selectedChat.recipientId || selectedChat.id, formData);
  ...
  return (
    <div className="relative inline-block">
      <button
        ref={buttonRef}
        onClick={() => { setIsDropdownOpen(!isDropdownOpen); setIsPopupOpen(false); }}
      ...
      {isDropdownOpen && createPortal(
        <div ref={dropdownRef} style={{ top: dropdownCoords.top, left: dropdownCoords.left }}
          ...
          <div className="max-h-[400px] overflow-y-auto">
            {Array.isArray(chats) && chats.length === 0 && !isLoading && <p className="p-4 text-center text-sm text-gray-400">No conversations</p>}
            ...
            {Array.isArray(chats) && chats.map((chat) => (
              <div key={chat.id} onClick={() => handleSelectChat(chat)}
                ...
                <img src={chat.img || '/avatar.png'} className="w-12 h-12 rounded-full object-cover" alt="" />
                <div className="flex-1 overflow-hidden">
                  <h5 className="font-semibold text-sm text-gray-900 dark:text-white truncate">{chat.name}</h5>
      ...
      {isPopupOpen && selectedChat && createPortal(
        <div className="fixed bottom-0 right-4 sm:right-8 w-full sm:w-[380px] h-[500px] bg-white dark:bg-slate-900 rounded-t-xl shadow-2xl border flex flex-col z-[9998] overflow-hidden">
          <div className="bg-white dark:bg-slate-800 p-3 border-b dark:border-slate-700 flex justify-between items-center shadow-sm">
            <span className="font-bold text-sm dark:text-white truncate">{selectedChat.name}</span>
            <div className="flex gap-1">
              <button onClick={() => { setIsPopupOpen(false); navigate('/chat'); }} className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-md"><FiMaximize2 size={14} /></button>
              <button onClick={() => setIsPopupOpen(false)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-md"><FiX size={16} /></button>
            </div>
          </div>
          <div className="flex-1 bg-gray-50 dark:bg-slate-950 overflow-hidden">
            <ChatWindow currentChat={selectedChat} messageText={messageText} setMessageText={setMessageText} onSendMessage={handleSendMessage} />
          </div>
        </div>, document.body
      )}
    </div>

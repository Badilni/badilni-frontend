...
const ChatInfoPanel = ({ viewMode, currentChat, isInfoOpen, setIsInfoOpen }) => {
  return (
    <div className={`h-full bg-[var(--whiteBackground)] dark:border-l dark:border-[var(--border-color)] flex flex-col shrink-0 transition-all duration-300 fixed lg:static right-0 top-0 z-40 shadow-xl lg:shadow-none ${
      isInfoOpen
        ? 'w-full sm:w-80 lg:w-[28%] xl:w-1/4 p-4 md:p-6 translate-x-0 opacity-100 flex'
        : 'w-0 lg:w-0 p-0 translate-x-full lg:translate-x-0 opacity-0 pointer-events-none hidden lg:hidden'
    } ${viewMode === 'info' ? 'flex' : ''}`}>

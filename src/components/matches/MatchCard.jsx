...
function LinkedCard({ item, type, owner, isOwn, onOwnerClick }) {
  ...
  const route = isListing ? `/offers/${item._id ?? item.id}` : `/requests/${item._id ?? item.id}`

  const accent = isListing
    ? { bg: 'bg-blue-50 dark:bg-blue-950/30', border: 'border-blue-100 dark:border-blue-900', badge: 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-400', tag: 'bg-blue-100/60 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' }
    : { bg: 'bg-emerald-50 dark:bg-emerald-950/30', border: 'border-emerald-100 dark:border-emerald-900', badge: 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-400', tag: 'bg-emerald-100/60 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' }

  return (
    <div className={`rounded-xl border ${accent.bg} ${accent.border} overflow-hidden`}>
      {/* Owner row */}
      <button
        type="button"
      ...
      >
        ...
        {isOwn && (
          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0 ${accent.badge}`}>
      ...
      >
        <p className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${accent.badge.split(' ').slice(-2).join(' ')}`}>
        ...
        {item.tags?.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {item.tags.slice(0, 3).map((tag) => (
              <span key={tag} className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${accent.tag}`}>
                #{tag}
              </span>
            ))}
            {item.tags.length > 3 && (
              <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded opacity-60 ${accent.tag}`}>
                +{item.tags.length - 3}
              </span>
            )}
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          {match.aiReasoning && (
            <div className="bg-[var(--background-light)] dark:bg-slate-800/50 rounded-xl p-3 flex gap-2">
              <svg className="w-3.5 h-3.5 shrink-0 mt-0.5 text-[var(--primary-light)]" fill="currentColor" viewBox="0 0 24 24">

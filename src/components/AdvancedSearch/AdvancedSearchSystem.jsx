        ...
        >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 h-full pl-3 pr-2.5 rounded-l-full  font-semibold font-mono tracking-wider transition-all duration-300 whitespace-nowrap border-r border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 ${
              mode === 'smart' ? 'text-violet-600 dark:text-violet-400' : 'text-blue-600 dark:text-blue-400'
            }`}
          >
            {activeMode.icon}
          ...
          <input
            ...
            placeholder={mode === 'smart' ? 'Describe what you need...' : 'Search...'}
          ...
          <div className="absolute top-[calc(100%+8px)] left-0 w-52 rounded-xl bg-white dark:bg-slate-800 shadow-xl border border-slate-100 dark:border-slate-700 py-1.5 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
            {MODES.map((m) => {
              ...
              return (
                ...
                >
                  <span className={`mt-0.5 shrink-0 ${isActive ? (m.key === 'smart' ? 'text-violet-500' : 'text-blue-500') : 'text-slate-400'}`}>
                    {m.icon}
                  </span>
                  <div>
                    <div className={`text-xs font-bold uppercase tracking-wide ${isActive ? (m.key === 'smart' ? 'text-violet-600 dark:text-violet-400' : 'text-blue-600 dark:text-blue-400') : 'text-slate-700 dark:text-slate-300'}`}>
                      {m.label}
                    </div>
          ...
          return (
            ...
            >
              ...
              <div className="relative flex items-start gap-3">
                ...
                <div>
                  <p className={`text-sm font-bold ${isActive ? 'text-white' : 'text-slate-900 dark:text-white'}`}>
                    {m.label}
                  </p>
                  <p className={`mt-0.5 text-xs leading-relaxed ${isActive ? 'text-white/75' : 'text-slate-500 dark:text-slate-400'}`}>
                    {m.description}
                  </p>
                </div>
              </div>
      ...
      <form onSubmit={onSubmit} className="group relative">
        <div
          className={`relative flex items-center overflow-hidden rounded-2xl border bg-[var(--whiteBackground)] dark:bg-slate-900 shadow-sm transition-all duration-300 ${activeMode.ring} ${
            mode === 'smart' ? 'border-violet-200 dark:border-violet-800/60' : 'border-slate-200 dark:border-slate-700'
          }`}
        >
          <div className={`absolute left-0 top-0 h-full w-1 rounded-l-2xl bg-gradient-to-b ${activeMode.gradient}`} />

          <span className={`ml-4 shrink-0 transition-colors duration-200 text-slate-400 ${activeMode.textFocus}`}>
            {activeMode.icon}
          </span>
        ...
        <p className={`mt-2 ml-1 text-xs transition-colors duration-200 ${
          mode === 'smart' ? 'text-violet-500 dark:text-violet-400' : 'text-slate-400 dark:text-slate-500'
        }`}>
          {mode === 'smart'

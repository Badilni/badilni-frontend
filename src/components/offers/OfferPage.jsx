      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <div className="bg-[var(--whiteBackground)] dark:bg-slate-900 rounded-3xl p-8 border border-gray-100 dark:border-slate-800 shadow-sm">
            <div
              className="flex items-center justify-between gap-4 p-4 bg-gray-50 dark:bg-slate-800 rounded-2xl flex-wrap cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-700/50 transition-colors"
              onClick={() => {
                if (offer.user) {
                  const ownerId = offer.user._id || offer.user.id
                  if (currentUser?._id === ownerId || currentUser?.id === ownerId) {
                    navigate('/profile')
                  } else {

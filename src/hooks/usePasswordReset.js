...
export const usePasswordReset = (email, verificationCode, onSuccess) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const validationResult = passwordResetSchema.safeParse({ password, confirmPassword });
    ...
    if (!email || !verificationCode) {
      setError("Session expired or missing data. Please start over from the email step.");
      return;
    }
    ...
    try {
      ...
      console.log("🚀 Sending FIXED Data to Backend:", { email, code: formattedCode, password, confirmPassword });

      const response = await axios.patch('http://localhost:3000/api/v1/auth/reset-password', {
        email: email,
        code: formattedCode,
        password: password,

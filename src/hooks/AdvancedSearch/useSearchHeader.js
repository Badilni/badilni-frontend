...
export const extractCollection = (payload, keys = []) => {
  ...
  const fallbackKeys = ['users', 'skillListings', 'serviceRequests', 'results', 'items']
...
export default function useSearchHeader() {
  ...
  const queryKeyword = searchParams.get('q') || searchParams.get('keyword') || ''
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        if (filterType === 'people') {
          ...
          setSearchResults(activeUsers.map((user) => ({ kind: 'people', data: user })))
          setTotalPages(resData?.pagination?.totalPages || resData?.totalPages || 1)
          setTotalResults(resData?.pagination?.totalCount || resData?.totalCount || activeUsers.length)
          return
        }

        if (filterType === 'offers') {
          const resData = await fetchOffers({ ...textParam, page: queryPage, limit: SEARCH_LIMIT })
          const offersList = extractCollection(resData, ['skillListings', 'data'])
          setSearchResults(offersList.map((offer) => ({ kind: 'offers', data: offer })))
          setTotalPages(resData?.pagination?.totalPages || resData?.totalPages || 1)
          setTotalResults(resData?.pagination?.totalCount || resData?.totalCount || offersList.length)
          return
        }

        if (filterType === 'requests') {
          const resData = await fetchServiceRequests({ ...textParam, page: queryPage, limit: SEARCH_LIMIT })
          const requestsList = extractCollection(resData, ['serviceRequests', 'data'])
          setSearchResults(requestsList.map((request) => ({ kind: 'requests', data: request })))
          setTotalPages(resData?.pagination?.totalPages || resData?.totalPages || 1)
          setTotalResults(resData?.pagination?.totalCount || resData?.totalCount || requestsList.length)
          return
        }

        // All — fetch all three in parallel
        const [usersRes, offersRes, requestsRes] = await Promise.allSettled([
          fetchUsersService({ keyword, page: queryPage, limit: SEARCH_LIMIT, sort: 'averageRating' }),
          fetchOffers({ ...textParam, page: queryPage, limit: SEARCH_LIMIT }),
          fetchServiceRequests({ ...textParam, page: queryPage, limit: SEARCH_LIMIT }),
        ])

        const userItems = usersRes.status === 'fulfilled'
          ? extractCollection(usersRes.value, ['users', 'data']) : []
        const offerItems = offersRes.status === 'fulfilled'
          ? extractCollection(offersRes.value, ['skillListings', 'data']) : []
        const requestItems = requestsRes.status === 'fulfilled'
        ...
        const normalized = [
          ...
          ...requestItems.map((request) => ({ kind: 'requests', data: request })),
        ]
  ...
  const handleFilterAll = () =>
    setSearchParams({ q: queryKeyword, page: '1', type: 'all', mode: searchMode })
  const handleFilterPeople = () =>
    setSearchParams({ q: queryKeyword, page: '1', type: 'people', mode: searchMode })
  const handleFilterOffers = () =>
    setSearchParams({ q: queryKeyword, page: '1', type: 'offers', mode: searchMode })
  const handleFilterRequests = () =>
    setSearchParams({ q: queryKeyword, page: '1', type: 'requests', mode: searchMode })

  const handlePageChange = (pageAction) => {
    const nextPage = typeof pageAction === 'function' ? pageAction(queryPage) : pageAction
    setSearchParams({ q: queryKeyword, page: String(nextPage), type: currentFilter, mode: searchMode })
  }

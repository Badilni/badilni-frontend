...
export default function useSearchHeader() {
  ...
  useEffect(() => {
    if (!searchParams.has('keyword') && !searchParams.has('page') && !searchParams.has('type')) {
      return;
    }
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        if (resData) {
          const usersList = resData.data?.users || resData.users || resData.data || (Array.isArray(resData) ? resData : []);

          setSearchResults(usersList);
          setTotalPages(resData.pagination?.totalPages || resData.totalPages || 1);
          setTotalResults(resData.pagination?.totalCount || resData.totalCount || usersList.length || 0);
        } else {
          setSearchResults([]);
          setTotalResults(0);
          setTotalPages(1);
  ...
  const handlePageChange = (pageAction) => {
    ...
    setSearchParams({ keyword: queryKeyword, page: nextPage, type: currentFilter });
  };
